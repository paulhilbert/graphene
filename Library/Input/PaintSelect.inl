inline PaintSelect::PaintSelect(FW::VisualizerHandle::Ptr handle, unsigned int radius, unsigned int subdivisions) : SelectionMethod(handle), m_radius(radius), m_subdivisions(subdivisions), m_numPoints(subdivisions+1) {
}

inline PaintSelect::~PaintSelect() {
}

inline void PaintSelect::init(const Eigen::Vector4f& color) {
...
inline void PaintSelect::uploadCircle(const Eigen::Vector4f& color) {
	std::vector<Eigen::Vector3f> points(m_numPoints);
	std::vector<Eigen::Vector4f> colors(m_numPoints, color);
	float radius = static_cast<float>(m_radius);
	int idx = 0;
	points[idx++] = Eigen::Vector3f::Zero();
	for (unsigned int i = 0; i <= m_subdivisions; ++i) {
		double angle = 2.0 * M_PI * static_cast<double>(i) / m_subdivisions;
